...
{
    public class SaveableEntityBuilding : MonoBehaviour, ISaveIDBuilding
    {
        [Serializable]
        ...
        public struct ObjectMetadata
        {
            ...
            public string parentID;   // ID of the next higher SaveableEntityBuilding 
            public string parentName; // Used to set the parent correct, may not a SaveableEntityBuilding, can be any gameObject
            public bool hasPrefab;
            public string prefabID;   // Prefab ID for instantiating this gameObject
            public string thisChildID; // ID for this in case this is a child of a SaveableEntityBuilding
        ...
        SaveableEntityBuilding m_parent = null;
        ...
        static Dictionary<string, SaveableEntityBuilding> m_allSaveables = new Dictionary<string, SaveableEntityBuilding>();
        ...
        private void Awake()
        {
            ...
            if (m_ID != "")
            {
                if (m_allSaveables.TryGetValue(m_ID, out SaveableEntityBuilding sav))
                {
        ...
        {
            List<SaveableEntityBuilding> childs = ObjectFinderBuilding.GetFirstChildLayerOfType<SaveableEntityBuilding>(gameObject);
        ...
        public void SetID(string ID)
        {
            string oldID = m_ID;
            m_ID = ID;
            if (m_allSaveables.TryGetValue(oldID, out SaveableEntityBuilding sav))
            {
                m_allSaveables.Remove(oldID);
            }
            if (m_allSaveables.TryGetValue(m_ID, out SaveableEntityBuilding sav2))
            {
                m_allSaveables.Remove(m_ID);
            }
            m_allSaveables.Add(m_ID, this);
        ...
        {
            SaveableEntityBuilding[] list = FindObjectsOfType<SaveableEntityBuilding>();

            foreach (var saveable in list)
            {

                foreach (var child in ObjectFinderBuilding.GetFirstChildLayerOfType<SaveableEntityBuilding>(saveable.gameObject))
                {
        ...
        public object SaveState()
        {
            ...
            ISaveableBuilding[] interfaces = GetComponents<ISaveableBuilding>();
            ...
            foreach (var saveable in ObjectFinderBuilding.GetFirstChildLayerOfType<SaveableEntityBuilding>(gameObject))
            {
        ...
        public static GameObject CreateFromSave(object state, SaveableEntityBuilding parent = null)
        {
            GameObject obj = null;
            if(state == null)
            {
            ...
            SaveableEntityBuilding savableOfInstance;
            if (stateDictionary.TryGetValue("ObjectMetadata", out object meta))
            {
                ...
                else
                {
                    ...
                    {
                        List<SaveableEntityBuilding> childsOfParent = ObjectFinderBuilding.GetFirstChildLayerOfType<SaveableEntityBuilding>(parent.gameObject);
                    ...
                    {
                        SaveableEntityBuilding o = FindID(metadata.thisID);
                        if (o != null)
                            obj = o.gameObject;
                ...
                if (obj == null)
                    goto warningMessage;
                savableOfInstance = obj.GetComponent<SaveableEntityBuilding>();
        ...
        {
            SaveableEntityBuilding obj = FindID(meta.thisID);
            if (obj != null)
                return obj.gameObject;
            GameObject prefab = SaveablePrefabsBuilding.GetPrefab(meta.prefabID);
            if (prefab == null)
                return null;
            GameObject newObj = Instantiate(prefab);
            SaveableEntityBuilding newSav = newObj.GetComponent<SaveableEntityBuilding>();
        ...
        static void SetupInstantiated(ObjectMetadata meta, SaveableEntityBuilding newSav)
        {
            newSav.SetID(meta.thisID);
            newSav.PrefabChildIdentifier();
            if (meta.deletedChilds.Count > 0)
            {
                List<SaveableEntityBuilding> childs = ObjectFinderBuilding.GetFirstChildLayerOfType<SaveableEntityBuilding>(newSav.gameObject);
        ...
        void DestroyChild(string childID, List<SaveableEntityBuilding> childs = null)
        {
            
            if (childs == null)
                childs = ObjectFinderBuilding.GetFirstChildLayerOfType<SaveableEntityBuilding>(gameObject);
            for (int i = 0; i < childs.Count; ++i)
            {
        ...
        void AttachToParent(SaveableEntityBuilding parent)
        {
            if (parent == null)
                return;
            if(m_parent != null)
           ...
           /* List<SaveableEntityBuilding> childsOfParent = ObjectFinderBuilding.GetFirstChildLayerOfType<SaveableEntityBuilding>(parent.gameObject);
            ...
            GameObject parentObj = ObjectFinderBuilding.GetFirstChildWithNameRecusive(parent.gameObject, m_metadata.parentName);
            if (parentObj != null)
            {
                /*for (int i = 0; i < parentObj.transform.childCount; ++i)
                {
                    ...
                    {
                        SaveableEntityBuilding childS = child.GetComponent<SaveableEntityBuilding>();
                        if (childS != null)
                ...
                List<ISaveableBuilding> signalReceiver = ObjectFinderBuilding.GetFirstParentOfType<ISaveableBuilding>(parentObj);
            ...
            else
            {
                ...
                List<ISaveableBuilding> signalReceiver = ObjectFinderBuilding.GetFirstParentOfType<ISaveableBuilding>(parent.gameObject);
                foreach (var receiver in signalReceiver)
        ...
        // Find the SaveableEntityBuilding with the identification string 
        // Use this in the ISaveable interface methode: PostInstantiation(...) to build relations between objects
        public static SaveableEntityBuilding FindID(string id)
        {
            
            m_allSaveables.TryGetValue(id, out SaveableEntityBuilding val); 
            return val;
        }
        ...
        void PostLoad()
        {
            if (m_dictionary == null)
            {
                ...
                //Debug.LogWarning("m_dictionary is null, did you forgot to call SaveableEntityBuilding::CreateFromSave() first?");
                return;
            }
            if (m_metadata.parentID != "")
            {
                foreach (var saveavle in FindObjectsOfType<SaveableEntityBuilding>())
                {
                    if (saveavle.m_ID == m_metadata.parentID)
                    {
                        // Parent Saveable found
                        // Search for exact parent Object...
                        ISaveableBuilding[] possibleEventReceiver = saveavle.gameObject.GetComponents<ISaveableBuilding>();

                        GameObject parent = ObjectFinderBuilding.GetFirstChildWithNameRecusive(saveavle.gameObject, m_metadata.parentName);
                        if (parent != null)
                        {
                                ...
                                if (child.name == transform.name)
                                {
                                    ...
                                    if (childS != null)
                                        ...
                                        //    m_addToAll.Add(childS);
                                         ...
                                         //   m_toDelete.Add(this);
                                            m_ID = "";
                                            foreach(var childOfTHis in GetComponentsInChildren<SaveableEntityBuilding>())
                                            {
                            ...
                            transform.SetParent(parent.transform);
                            List<ISaveableBuilding> signalReceiver = ObjectFinderBuilding.GetFirstParentOfType<ISaveableBuilding>(parent);
                            foreach (var receiver in signalReceiver)
        ...
        void TransformUpdate()
        {
            if (m_dictionary == null || m_parent != null)
                return;
            TransformUpdate_recursive(new List<SaveableEntityBuilding> { this });
        }
        void TransformUpdate_recursive(List<SaveableEntityBuilding> roots)
        {
            for (int i = 0; i < roots.Count; ++i)
            {
                List<SaveableEntityBuilding> childs = ObjectFinderBuilding.GetFirstChildLayerOfType<SaveableEntityBuilding>(roots[i].gameObject);

                TransformUpdate_recursive(childs);
                foreach (SaveableEntityBuilding child in childs)
