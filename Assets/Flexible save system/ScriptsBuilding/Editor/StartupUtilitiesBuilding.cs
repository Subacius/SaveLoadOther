...
{
    [InitializeOnLoad]
    public class StartupUtilitiesBuilding : EditorWindow
    {
        ...
        static StartupUtilitiesBuilding()
        {
        ...
        static void CheckForIDConflicts_prefabs()
        {
            ...
            List<SaveablePrefabsEditorBuilding.ConflictData> conflicts = SaveablePrefabsEditorBuilding.GetIDConflicts();
            if (conflicts.Count > 0)
            {
                ...
                {
                    SaveablePrefabsEditorBuilding.ResolveIDConflicts();
                }
            }
        }
        static void CheckForIDConflicts_scriptableObjects()
        {
            ...
            for (int i = 0; i < objs.Length; ++i)
            {
                ...
                {
                    if (objs[i] is ISaveIDBuilding)
                    {
                        ISaveIDBuilding saveID_interface = objs[i] as ISaveIDBuilding;
            ...
            if (conflicts.Count > 0)
            {
                ...
                if (IDConflictPopup("Scriptable Objects ID Conflict", text))
                {
                    ...
                    foreach (var conflict in conflicts)
                    {
                        ...
                        {
                            if (conflict.Value is ISaveIDBuilding)
                            {
                                ISaveIDBuilding saveID_interface = conflict.Value as ISaveIDBuilding;
        ...
        static void CheckForIDConflicts_GameObject()
        {
            ...
            for (int i = 0; i < objs.Length; ++i)
            {
                ...
                Component[] saveIDs = objs[i].GetComponents(typeof(ISaveIDBuilding));
                for (int j = 0; j < saveIDs.Length; ++j)
                {
                    if (saveIDs[j] is ISaveIDBuilding)
                    {
                        ISaveIDBuilding saveID_interface = saveIDs[j] as ISaveIDBuilding;
                        ...
                        if (inAllList != null)
                        {
                            Component[] ids = inAllList.GetComponents(typeof(ISaveIDBuilding));
            ...
            if (conflicts.Count > 0)
            {
                ...
                if (IDConflictPopup("GameObjects ID Conflict", text))
                {
                    ...
                    foreach (var conflict in conflicts)
                    {
                        foreach (var elem in conflict.Value)
                        {
                            Component[] saveIDs = elem.GetComponents(typeof(ISaveIDBuilding));
                            for (int j = 0; j < saveIDs.Length; ++j)
                            {
                                if (saveIDs[j] is ISaveIDBuilding)
                                {
